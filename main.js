...
const createProjectCards = () => {
    ...
    for (let i = 0; i < projects.length; i++) {
        if (projects[i].available === true) {
            ...
            domString += `<header class="projectsCard" id="projectsTitle"><h2>${projects[i].title}</h2></header>`;
            domString += `<section class="projectImages"><img src=${projects[i].screenshot} alt="Image of ${projects[i].title}"></section>`;
            domString += `</div>`;
        } else {
            console.log(`${projects[i].title} not available`);
        }
    }
